...
if (dir.includes("400")) {
  $("#errorMessage")
    .html(
      '<h1 class="mt-3">400 Bad Request</h1><h3 class="mt-3">Warning: The client should not repeat this request without modification.</h3>'
    )
    .show();
} else if (dir.includes("401")) {
  $("#errorMessage")
    .html(
      '<h1 class="mt-3">401 Unauthorized</h1><h3 class="mt-3">Client Request Has Not Been Completed Because It Lacks Valid Authentication Credentials for the Requested Resource.</h3>'
    )
    .show();
} else if (dir.includes("403")) {
  $("#errorMessage")
    .html(
      '<h1 class="mt-3">403 Forbidden</h1><h3 class="mt-3">You do not have access to this resource.</h3>'
    )
    .show();
